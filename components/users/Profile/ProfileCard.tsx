...
export function ProfileCard({ name, username, bio, avatarUrl, status, joinedAt }: ProfileCardProps) {
  return (
    <Box sx={{ width: 600, p: 2, borderRadius: 2, boxShadow: 1 }}>
      <Box sx={{ display: 'flex', alignItems: 'center' }}>
        ...
        <Box>
          <Box component="div" sx={{ fontWeight: 700 }}>{name}</Box>
          <Box component="div" sx={{ color: 'text.secondary' }}>@{username}</Box>
        </Box>
      </Box>
