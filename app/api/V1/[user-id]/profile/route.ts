...
/* -------------------------------- route --------------------------------- */
export async function GET(_req: NextRequest, ctx: { params: Promise<{ 'user-id': string }> }) {
  try {
    ...
    if (viewerId !== targetUserId) {
      return NextResponse.json(
        { error: 'Forbidden: can only sync your own profile' },
    ...
    return NextResponse.json(
      { error: 'Failed to sync and fetch profile' },
